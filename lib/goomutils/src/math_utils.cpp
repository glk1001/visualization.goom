...
LogIncreasingFunction::LogIncreasingFunction(const double amp, const double xm, const double xStrt)
  : IncreasingFunction(xm, 10000000000)
  , amplitude(amp),
    xmin(xm)
  , xStart(xStrt)
{
...
ExpDampingFunction::ExpDampingFunction(
    const double amp,
...
SineWaveMultiplier::SineWaveMultiplier(const float freq, const float lwr, const float upr, const float x0)
  : rangeMapper{ -1, +1 }
  , frequency{ freq }
  , lower{ lwr }
  , upper{ upr }
