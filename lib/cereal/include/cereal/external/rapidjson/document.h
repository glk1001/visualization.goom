...
#endif
  template<SizeType N>
  GenericStringRef(const CharType (&str)[N]) CEREAL_RAPIDJSON_NOEXCEPT : s(str), length(N - 1)
  {
  }
...
public:
  ...
  /*! \return (rhs == lhs)
     */
  template<typename T>
  friend CEREAL_RAPIDJSON_DISABLEIF_RETURN((internal::IsGenericValue<T>), (bool)) operator==(
...
public:
  ...
  GenericDocument& Populate(Generator& g)
  {
    ...
    if (g(*this))
    {
      CEREAL_RAPIDJSON_ASSERT(stack_.GetSize() ==
