...
{
  GOOM_LOG_DEBUG("Initializing ifs.");
  ...
  Fractal->Col = pcg32() % (width * height);/* modif by JeKo */
  ...
  for (int i = 0; i < 5 * MAX_SIMI; i++) {
    ...
    GOOM_LOG_DEBUG(
        "simi[%d]: c_x = %f, c_y = %f, r = %f, r2 = %f, A = %f, A2 = %f, Ct = %d, St = %d, Ct2 = %d, St2 = %d, Cx = %d, Cy = %d, R = %d, R2 = %d ",
        i, cur.c_x, cur.c_y, cur.r, cur.r2, cur.A, cur.A2, cur.Ct, cur.St, cur.Ct2, cur.St2, cur.Cx, cur.Cy, cur.R,
        cur.R2);
  }
}
