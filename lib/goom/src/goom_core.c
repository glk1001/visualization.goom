...
static void choose_a_goom_line(PluginInfo *goomInfo, float *param1, float *param2, int *couleur, int *mode,
...
*/
guint32* goom_update(PluginInfo *goomInfo, const gint16 data[NUM_AUDIO_SAMPLES][AUDIO_SAMPLE_LEN], int forceMode,
...
/* on verifie qu'il ne se pas un truc interressant avec le son. */
  ...
  if (goomInfo->update.lockvar == 0) {
    ...
    if (goomInfo->sound.timeSinceLastGoom == 0) {
      ...
      if (!goomInfo->curGState->drawScope) { /* DOESN'T MAKE SENSE - SET BELOW */
      ...
      /* if (goomInfo->update.goomvar % 1 == 0) */
      {
        ...
        if ((goomInfo->update.zoomFilterData.mode == WATER_MODE) || (goomInfo->update.zoomFilterData.mode == YONLY_MODE)
            || (goomInfo->update.zoomFilterData.mode == AMULETTE_MODE)) {
          goomInfo->update.zoomFilterData.middleX = goomInfo->screen.width / 2;
          goomInfo->update.zoomFilterData.middleY = goomInfo->screen.height / 2;
        }
        ...
        if ((signed int) newvit < goomInfo->update.zoomFilterData.vitesse) { /* on accelere */
          ...
          if (((newvit < STOP_SPEED - 7) && (goomInfo->update.zoomFilterData.vitesse < STOP_SPEED - 6)
  ...
  if ((goomInfo->sound.speedvar < 0.01f) && (goomInfo->update.zoomFilterData.vitesse < STOP_SPEED - 4)
      && (goomInfo->cycle % 16 == 0)) {
  ...
  if (pzfd != NULL) {
    ...
    if (((goomInfo->sound.timeSinceLastGoom == 0) && (goomInfo->sound.totalgoom < 2)) || (forceMode > 0)) {
   ...
   */
  {
    ...
    if (goomInfo->update.timeOfTitleDisplay) {
      goom_draw_text(goomInfo->p1, goomInfo->screen.width, goomInfo->screen.height, goomInfo->screen.width / 2,
      ...
      if (goomInfo->update.timeOfTitleDisplay < 4) {
        goom_draw_text(goomInfo->p2, goomInfo->screen.width, goomInfo->screen.height, goomInfo->screen.width / 2,
            goomInfo->screen.height / 2 + 7, goomInfo->update.titleText,
            ((float) (190 - goomInfo->update.timeOfTitleDisplay) / 10.0f), 1);
      }
    }
  ...
  } else if ((goomInfo->cycle % 80 == 0) && (goom_irand(goomInfo->gRandom, 5) == 0) && goomInfo->update.lineMode) {
    goomInfo->update.lineMode--;
  }
  ...
  if ((goomInfo->cycle % 120 == 0) && (goom_irand(goomInfo->gRandom, 4) == 0) && (goomInfo->curGState->drawScope)) {
    if (goomInfo->update.lineMode == 0)
      ...
      goomInfo->update.lineMode--;
      choose_a_goom_line(goomInfo, &param1, &param2, &couleur1, &mode, &amplitude, goomInfo->update.stop_lines);
  ...
  if ((goomInfo->update.lineMode != 0) || (goomInfo->sound.timeSinceLastGoom < 5)) {
    ...
    const int rand1_3 = goom_irand(goomInfo->gRandom, 3);
    GOOM_LOG_DEBUG("rand1_3 = %d", rand1_3);
    if (((goomInfo->cycle % 121) == 9) && (rand1_3 == 1)
        && ((goomInfo->update.lineMode == 0) || (goomInfo->update.lineMode == goomInfo->update.drawLinesDuration))) {
      float param1, param2, amplitude;
      int couleur1, couleur2;
      int mode;

      GOOM_LOG_DEBUG("goomInfo->cycle %% 121 etc.: goomInfo->cycle = %d, rand1_3 = %d", goomInfo->cycle, rand1_3);
      choose_a_goom_line(goomInfo, &param1, &param2, &couleur1, &mode, &amplitude, goomInfo->update.stop_lines);
      couleur2 = 5 - couleur1;
...
static void update_message(PluginInfo *goomInfo, const char *message)
{
  ...
  if (message) {
    ...
    goomInfo->update_message.affiche = goomInfo->screen.height + goomInfo->update_message.numberOfLinesInMessage * 25
        + 105;
  ...
  if (goomInfo->update_message.affiche) {
    ...
    while (!fin) {
      ...
      goom_draw_text(goomInfo->p1, goomInfo->screen.width, goomInfo->screen.height, goomInfo->screen.width / 2, pos,
          message, ecart, 1);
      message = ++ptr;
      i++;
    }
